...
namespace CALICE
{
  ...
  ShapedConversionProcessor::ShapedConversionProcessor() : Processor("ShapedConversionProcessor")
  {
    ...
    // TODO Update naming on this to aux_shaped_conversion
  ...
  void ShapedConversionProcessor::init()
  {
    ...
    _maxSubHits = 0;
    _maxHits = 0;
    _writeHisto_mu_thr = 2; // Legacy from first conversion // May be a steering file? Or maybe not if params are stored in ttrees
    _writeHisto = false;
    ...
    // _landau_mpv_cor = _fitParams->Branch("_landau_mpv_cor",&_landau_mpv_cor,"_landau_mpv_cor/D"); 
    // _landau_sigma = _fitParams->Branch("_landau_sigma",&_landau_sigma,"_landau_sigma/D"); 
    // _langauss_c =   _fitParams->Branch("_langauss_c",&_langauss_c,"_langauss_c/D"); 
    // _chi2ndf =      _fitParams->Branch("_chi2ndf",&_chi2ndf,"_chi2ndf/D"); 


    // This is segfaulting!
    // //auto *_fitParams = new TTree("FitParams","Energy cell fit parameters");
    // TTree* _fitParams = new TTree("FitParams","Energy cell fit parameters");
    // _fitParams->Branch("_gauss_c",&_gauss_c,"_gauss_c/D"); 
    // _fitParams->Branch("_gauss_mu",&_gauss_mu,"_gauss_mu/D"); 
    // _fitParams->Branch("_gauss_sigma",&_gauss_sigma,"_gauss_sigma/D"); 
    // _fitParams->Branch("_landau_c",&_landau_c,"_landau_c/D"); 
    // _fitParams->Branch("_landau_mpv",&_landau_mpv,"_landau_mpv/D"); 
    // _fitParams->Branch("_landau_mpv_cor",&_landau_mpv_cor,"_landau_mpv_cor/D"); 
    // _fitParams->Branch("_landau_sigma",&_landau_sigma,"_landau_sigma/D"); 
    // _fitParams->Branch("_langauss_c",&_langauss_c,"_langauss_c/D"); 
    // _fitParams->Branch("_chi2ndf",&_chi2ndf,"_chi2ndf/D"); 
    
    // Fit histos, gauss, landau
    _gauss_c = TH1F("gauss_c", "Gauss constant distribution", 100, 0., 50.);
    _gauss_mu = TH1F("gauss_mu", "Gauss mu distribution", 400, 0.5, 3);
    _gauss_sigma = TH1F("gauss_sigma", "Gauss sigma distribution", 100, 0., 1);
    _landau_c = TH1F("landau_c", "Landau constant distribution", 100, 0., 50.);
    // For W confs
    //_landau_mpv = TH1F("landau_mpv", "Landau MPV distribution", 100, 0.00005, 0.00025);
    // for conf0
    _landau_mpv = TH1F("landau_mpv", "Landau MPV distribution", 400, 0.5, 3);
    _landau_mpv_cor = TH1F("landau_mpv_cor", "Landau Corrected MPV distribution", 400, 0.5, 3);
    _landau_sigma = TH1F("landau_sigma", "Landau sigma distribution", 100, 0., 1);
    _chi2ndf = TH1F("chi2ndf", "chi2ndf distribution", 200, 0., 20);
    // Langaus
    _langauss_c = TH1F("langauss_c", "Landau (x) Gaussian constant distribution", 100, 0., 6000.);
      ...
      for (unsigned int iy = 0; iy < _FixedPosXY.size(); iy++)
      {
        for (unsigned int iz = 0; iz < _FixedPosZ.size(); iz++)
        {
          ...
          energyHistos[ix][iy][iz] = new TH1F(ename.c_str(), ename.c_str(),
                                              100, 0., 10); // MIPs from 1st conversion
    ...
    _dir_params = _rootout->mkdir("fit_parameters");
    _dir_cells  = _rootout->mkdir("cells");
    ...
    for(unsigned int icol = 0; icol < _calorimInpCollections.size(); icol++)
    {
      if( std::find(cnames->begin(), cnames->end(), _calorimInpCollections.at(icol)) != cnames->end() )
      {
        ...
        if (inputCalorimCollection != 0){
          ...
          {
            CalorimeterHit *aCalorimHit = dynamic_cast<CalorimeterHit*>(inputCalorimCollection->getElementAt(i));

            int this_cellid = aCalorimHit->getCellID0();
            float this_cellPosX = aCalorimHit->getPosition()[0];
            float this_cellPosY = aCalorimHit->getPosition()[1];
            float this_cellPosZ = aCalorimHit->getPosition()[2];
            ...
            energyHistos[ix][iy][iz]->Fill(aCalorimHit->getEnergy());
  ...
  void ShapedConversionProcessor::end(){
   ...
   for (unsigned int iz = 0; iz < _FixedPosZ.size(); iz++)
   {
      for (unsigned int ix = 0; ix < _FixedPosXY.size(); ix++)
      {
        for (unsigned int iy = 0; iy < _FixedPosXY.size(); iy++)
        {
          if (energyHistos[ix][iy][iz]->GetEntries() > 20)
          {
           
            if (_MIPFitMode == 1) {fitGauss(energyHistos[ix][iy][iz]);}
            else if (_MIPFitMode == 2) {fitLandau(energyHistos[ix][iy][iz]);}
            else if (_MIPFitMode == 3) {fitLanGauss(energyHistos[ix][iy][iz]);}
            else
            {
              std::cout << "Invalid _MIPFitMode = " << std::to_string(_MIPFitMode) << "\n";
              break;
            }
            ...
            if (writeCounter < 20 && !_writeHisto)
            {
              ...
              writeCounter++;
              //std::cout << "Wrote " << writeCounter << " energy histos\n";
            }
            if (_writeHisto)
            {
  ...
  void ShapedConversionProcessor::fitGauss(TH1F* h)
  {
    ...
    // The TTree way
    // _gauss_c = f->GetParameter(0);
    // _gauss_mu = f->GetParameter(1);
    // if (f->GetParameter(1) > _writeHisto_mu_thr) {_writeHisto = true;}
    // _gauss_sigma = f->GetParameter(2);
    // _chi2ndf = f->GetChisquare()/f->GetNDF();
  ...
  void ShapedConversionProcessor::fitLandau(TH1F* h)
  {
    ...
    // The TTree way
    // _landau_c = f->GetParameter(0);
    // _landau_mpv = f->GetParameter(1);
    // double mpv_corrected = f->GetParameter(1) + 0.22278298 * f->GetParameter(2);
    // _landau_mpv_cor = mpv_corrected;
    // if (mpv_corrected > _writeHisto_mu_thr) {_writeHisto = true;}
    // _landau_sigma = f->GetParameter(2);
    // _chi2ndf = f->GetChisquare()/f->GetNDF();
  ...
  // Landau (x) Gauss by Adrián - Likely from
  ...
  void ShapedConversionProcessor::fitLanGauss(TH1F* h)
  {
    ...
    _langauss_c.Fill(f->GetParameter(2));
    _landau_mpv.Fill(f->GetParameter(1));
    ...
    // The TTree way
    // _langauss_c = f->GetParameter(2);
    // _landau_mpv = f->GetParameter(1);
    // double mpv_corrected = f->GetParameter(1) + 0.22278298 * f->GetParameter(0);
    // _landau_mpv_cor = mpv_corrected;
    // if (mpv_corrected > _writeHisto_mu_thr) {_writeHisto = true;}
    // _landau_sigma = f->GetParameter(0);
    // _gauss_sigma = f->GetParameter(3);
    // // _chi2ndf = f->GetChisquare()/f->GetNDF();
    // _chi2ndf = chisqr / ndf;
